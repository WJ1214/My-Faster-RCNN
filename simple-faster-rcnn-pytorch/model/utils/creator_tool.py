...
class ProposalTargetCreator(object):
    ...
    def __call__(self, roi, bbox, label,
        ...
        # n_sample为最后输出的ROI，pos_ratio为将ROIs标记为前景的比例，（该函数为每个ROI分配相应GT的偏移值以及相应GT的标签？）
        ...
        * :math:`L` is number of object classes possibly including the \
        ...
        roi = np.concatenate((roi, bbox), axis=0)   # 为什么要连接起来？

        pos_roi_per_image = np.round(self.n_sample * self.pos_ratio)
        iou = bbox_iou(roi, bbox)
        gt_assignment = iou.argmax(axis=1)   # 每个Roi与所有GT的最大iou的索引（用来分配ROI的标签）
        max_iou = iou.max(axis=1)            # 每个ROI与所有GT的iou的最大值
        ...
        pos_roi_per_this_image = int(min(pos_roi_per_image, pos_index.size))   # 取预定比率Roi数量和IOU大于积极值的ROi的最小值
        if pos_index.size > 0:
            pos_index = np.random.choice(
                pos_index, size=pos_roi_per_this_image, replace=False)   # 从备选pos_index中随机选出pos_roi_per_this_image个数的ROI
        ...
        keep_index = np.append(pos_index, neg_index)  # 符合条件的正样本和负样本都保存了
        gt_roi_label = gt_roi_label[keep_index]
        gt_roi_label[pos_roi_per_this_image:] = 0  # negative labels --> 0
        sample_roi = roi[keep_index]          # 取正负样本
